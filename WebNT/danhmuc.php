...
<body>
	...
	if (isset($_GET['iddanhmuc'])) {
		$sql="SELECT * FROM danhmuc WHERE id=".$_GET['iddanhmuc'];
		$ketqua=mysqli_query($conn,$sql);
		$row=mysqli_fetch_assoc($ketqua);
	}	
	
	?>
		<div class="container-fluid " style="background: #F5F5F5 ;height: 320px">
			<div class="container" >
				<div class="row" >
					...
					<div class="col-7 " >
						<img src="img/<?php echo $row['anh']; ?>" style="width: 80%; height: 70%"alt="" class="rounded "  >
					</div>
				</div>
			</div>
		</div>
				...
				<div class="row  mt-3" >
					...
					<div class="col-10 text-right">
						<span style="display: inline-block;float: left">
							<form action="danhmuc.php?idphong=<?php echo $_GET['idphong']; ?>&iddanhmuc=<?php echo $_GET['iddanhmuc']; ?>" method="POST" accept-charset="utf-8">
								Số sản phẩm
								<select name="soluong" class=""  >
									<?php 
										$result=mysqli_query($conn,'select count(id) as total from hanghoa where iddanhmuc='.$_GET['iddanhmuc']);
						...
						HIỆN CÓ 468 SẢN PHẨM <a href="#" title=""><span class="fa fa-th border" style="padding: 10px"></span></a><a href="#" title=""></span></a>
						<hr>
					</div>
					<div class="col-2">
						<div class="row">
								...
								?>
									<table>
										...
										<?php  
											...
											echo '<td width="90%"><a style="color: orange"href="phong.php?idphong='.$row_phong['id'].'" title=""><p style=""> '.$row_phong['tenphong'].'</p></a></td>';
											}
											else {
											echo '<td width="90%"><a style="color: black"href="phong.php?idphong='.$row_phong['id'].'" title=""><p style=""> '.$row_phong['tenphong'].'</p></a></td>';
											}
										?>	
										...
										<?php  
											...
											echo '<div class="col-12 collapse show" style="border-left: 1px solid black" id="demo'.$row_phong['id'].'">';
											}
											else {
										
											echo '<div class="col-12 collapse " style="border-left: 1px solid black" id="demo'.$row_phong['id'].'">';
											}
										?>	
										<?php  
											...
											while($row_danhmuc=mysqli_fetch_assoc($ketqua_danhmuc)){
												if ($row_danhmuc['id']==$_GET['iddanhmuc']) {
										?>
												<p><a href="danhmuc.php?iddanhmuc=<?php echo $row_danhmuc['id']; ?>&idphong=<?php echo $row_phong['id']; ?>" title="" style="text-decoration: none;color: orange"><?php echo $row_danhmuc['tendanhmuc']; ?></a> </p>
												<?php
												}
												else {
												?>
						...
						<form action="danhmuc.php?#sanpham" method="GET" style="background: #FFF;">
							<div class="col-xs-3" style="">
								<div class="col-xs-12 text-center" style="height: 100%">
									<p class="">Khoảng giá</p>
									<input type="number" min="0"required name="gia"  style="width: 40%" placeholder="">
										-
									<input type="number" name="gialh"required min="1"  style="width: 40%;" placeholder="">
									<input type="hidden" name="idphong" value="<?php echo $_GET['idphong'] ?>" style="width: 40%;" placeholder="">
									<input type="hidden" name="iddanhmuc" value="<?php echo $_GET['iddanhmuc'] ?>" style="width: 40%;" placeholder="">
						...
						<div class="row">
							<?php  
								...
								if (isset($_GET['gia'])&&isset($_GET['gialh'])) {
										...
										$gia=$_GET['gia'];
										$gialh=$_GET['gialh'];
										$result=mysqli_query($conn,"SELECT count(id) as total FROM hanghoa Where dongia BETWEEN ".$gia." and ".$gialh." and iddanhmuc=".$_GET['iddanhmuc']);
										$row_tong1 =mysqli_fetch_assoc($result);
										$total_records=$row_tong1['total'];
										$total_page1=ceil($total_records/$limit);
										$start=($current_page-1)* $limit;
										$ketqua_hanghoa=mysqli_query($conn,"SELECT * FROM hanghoa Where dongia BETWEEN ".$gia." and ".$gialh." and iddanhmuc=".$_GET['iddanhmuc']." LIMIT $start,$limit");
									}

								else $ketqua_hanghoa=mysqli_query($conn,"SELECT * FROM hanghoa WHERE iddanhmuc=".$_GET['iddanhmuc']." LIMIT $start,$limit");	
								...
								<div class="col-6 text-center" >
									...
									if(isset($tktg)){
								 		...
								 		if ($current_page>1&&$total_page>1) {
								 			echo '<a class="btn btn-primary" href="danhmuc.php?page='.($current_page-1).'&gia='.$gia.'&gialh='.$gialh.'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'" title="">Prev</a> | ';
								 		}
								 		for ($i=1; $i <=$total_page ; $i++) { 
								 			if ($i==$current_page) {
								 				...
								 				echo '<a class="btn btn-primary" href="danhmuc.php?page='.$i.'&gia='.$gia.'&gialh='.$gialh.'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'" title="">'.$i.'</a> | ';

								 			}

								 		}
								 		if ($current_page<$total_page&& $total_page>1) {
								 			echo '<a class="btn btn-primary" href="danhmuc.php?page='.($current_page+1).'&gia='.$gia.'&gialh='.$gialh.'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'	" title="">Next</a>  ';
								 		}
								 	}
								 	else{
								 		if ($current_page>1&&$total_page>1) {
								 			echo '<a class="btn btn-primary" href="danhmuc.php?page='.($current_page-1).'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'" title="">Prev</a> | ';
								 		}
								 		for ($i=1; $i <=$total_page ; $i++) { 
								 			if ($i==$current_page) {
								 				echo '<span class="btn btn-primary">'.$i.'</span> | ';
								 				...
								 				echo '<a class="btn btn-primary" href="danhmuc.php?page='.$i.'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'" title="">'.$i.'</a> | ';

								 			}

								 		}
								 		if ($current_page<$total_page&& $total_page>1) {
								 			echo '<a class="btn btn-primary" href="danhmuc.php?page='.($current_page+1).'&idphong='.$_GET['idphong'].'&iddanhmuc='.$_GET['iddanhmuc'].'	" title="">Next</a>  ';
								 		}
								 	} 
