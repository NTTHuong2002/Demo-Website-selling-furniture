      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          while($ngay<32){
            ...
            $sql_donhang="SELECT (DAY(ngay)) AS month,(MONTH(ngay)) AS year, SUM(tongtien) AS gia FROM donhang WHERE (DAY(ngay)) = $ngay and MONTH(ngay)=MONTH(NOW())  and YEAR(ngay)=2021 GROUP BY DAY(ngay)";
        ...
        var options = {
          title: 'Thống kê doanh thu của năm 2020',
  ...
  <!--thống kê số lượng đơn hàng --> 
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          <?php  
            ...
            while($ngay<32){
              ...
              $sql_donhang="SELECT DAY(ngay) as month,COUNT(1) as dem, SUM(tongsanpham) as tong FROM donhang WHERE DAY(ngay)=$ngay and MONTH(ngay)=MONTH(NOW())  and YEAR(ngay)=2021  GROUP BY DAY(ngay) HAVING COUNT(1) > 0";
              $ketqua_donhang=mysqli_query($connect,$sql_donhang);
              if ( mysqli_num_rows($ketqua_donhang) < 1 ){
              ?>
              ['<?php echo $ngay; ?>', 0,0],
        ...
        var options = {
          title: 'Thống kê đơn hàng của năm 2020',
          hAxis: {title: 'Ngày',  titleTextStyle: {color: '#333'}},
          vAxis: {minValue: 0 }
        };
...
<body style="background: #F5F5F5">
      ...
      <div id="main1" style="margin-left: 20%">
       <div class="col-12"  id="trangchinh">
          ...
          <div class="row">
            <div class="col-12" >
              <h3>Thống kê số liệu theo ngày </h3>
              <table class="table  table-borderless"  style="border-spacing: 20px;border-collapse: separate;height: 700px">
                 <tr>
                  <td colspan="2" style="background: white "  height="50%">
